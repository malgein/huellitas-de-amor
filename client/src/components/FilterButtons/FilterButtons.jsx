...
const FilterMascotas = () => {
  ...
  <div className="relative" key={filter}>
            ...
            {filter === showDropdown && (
              <div className="absolute z-10 bg-white border rounded shadow mt-2 w-full">
                {options[filter].map((option) => (
  <button onClick={() => handleChange(filter, option)} className="block w-full text-left p-2 hover:bg-gray-100" key={option}>
      ...
      <div className="mt-4">
        <button className="bg-blue-500 hover:bg-blue-600 text-white p-2 rounded w-full" onClick={handleApplyFilters}>
          Aplicar Filtros
        </button>
