...
const ImagenesPerfil = ({ setImagenes }) => {
  ...
  const handleDrop = async (files) => {
    ...
    const validFiles = files.filter((file) => {
      ...
      return (
        allowedTypes.includes(file.type.toLowerCase()) && file.size <= maxSize
      );
    });
  ...
  return (
    <div>
      ...
      <div className="text-xs text-red-700 justify-center mb-[10px] text-center">
        {mensajeError}
      </div>
    </div>
  );
